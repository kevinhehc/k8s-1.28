...
type Controller struct {
	...
	// 通过--node-monitor-period 设置，默认为 5s，表示在 NodeController 中同步NodeStatus 的周期；
	...
	// 通过--node-monitor-grace-period 设置，默认 40s，表示在标记某个 node为 unhealthy 前，允许 40s 内该 node 无响应；
...
4、若启用了 TaintBasedEvictions 特性则启动一个 goroutine 调用 nc.doNoExecuteTaintingPass 处理
   nc.zoneNoExecuteTainter 队列中的 node，否则调用 nc.doEvictionPass 处理 nc.zonePodEvictor 队列中的 node；
...
func (nc *Controller) Run(ctx context.Context) {
	...
	/*
		...
		1、nc.taintManager.Run：处理 taintManager 中 nodeUpdateQueue 和 podUpdateQueue 中的 pod 以及 node，
		   若 pod 不能容忍 node 上的 taint 则将其加入到 taintEvictionQueue 中并最终会删除；
		2、nc.doNodeProcessingPassWorker：从 NodeLifecycleController 的 nodeUpdateQueue 取出 node，
		    （1）若启用 taintNodeByCondition 特性时根据 node condition 以及 node 是否调度为 node 添加对应的 NoSchedule taint 标签；
		    （2）调用 nc.reconcileNodeLabels 为 node 添加默认的 label；
		3、nc.doNoExecuteTaintingPass：处理 nc.zoneNoExecuteTainter 队列中的数据，根据 node 的 NodeReadyCondition 添加或移除对应的 taint；
		4、nc.doEvictionPass：处理 nc.zonePodEvictor 队列中的 node，将 node 上的 pod 进行驱逐；
		5、nc.monitorNodeHealth：持续监控 node 的状态，
		   当 node 处于异常状态时更新 node 的 taint 以及 node 上 pod 的状态或者直接驱逐 node 上的 pod，
		   此外还会为集群下的所有 node 划分 zoneStates 并为每个 zoneStates 设置对应的驱逐速率；
	*/
}

/*
NodeLifecycleController 中 nodeInformer 监听到 node 变化时会将其添加到 nodeUpdateQueue 中，
nc.doNodeProcessingPassWorker 主要是处理 nodeUpdateQueue 中的 node，为其添加合适的 NoSchedule taint 以及 label，其主要逻辑为：

1、从 nc.nodeUpdateQueue 中取出 node；
2、若启用了 TaintNodeByCondition feature-gates，调用nc.doNoScheduleTaintingPass

	    检查该 node 是否需要添加对应的 NoSchedule taint； nc.doNoScheduleTaintingPass 中的主要逻辑为：
	 2.1、从 nodeLister 中获取该 node 对象；
	 2.2、判断该 node 是否存在以下几种 Condition：
		(1) False 或 Unknown 状态的 NodeReady Condition；
		(2) MemoryPressureCondition；
		(3) DiskPressureCondition；
		(4) NetworkUnavailableCondition；
		(5) PIDPressureCondition；
		若任一一种存在会添加对应的 NoSchedule taint；
	 2.3、判断 node 是否处于 Unschedulable 状态，若为 Unschedulable 也添加对应的 NoSchedule taint；
	 2.4、对比 node 已有的 taints 以及需要添加的 taints，以需要添加的 taints 为准，
		调用 nodeutil.SwapNodeControllerTaint 为 node 添加不存在的 taints 并删除不需要的 taints；
...
/*
	当启用了 TaintBasedEvictions 特性时，通过 nc.monitorNodeHealth 检测到 node 异常时会将其加入到 nc.zoneNoExecuteTainter 队列中，
	nc.doNoExecuteTaintingPass 会处理 nc.zoneNoExecuteTainter 队列中的 node，并且会按一定的速率进行，
	 此时会根据 node 实际的 NodeCondition 为 node 添加对应的 taint，当 node 存在 taint 时，taintManager 会驱逐 node 上的 pod。
	  此过程中为 node 添加 taint 时进行了限速避免一次性驱逐过多 pod，在驱逐 node 上的 pod 时不会限速。
...
3、判断 NodeReadyCondition 的状态，若为 false，则为 node 添加 node.kubernetes.io/not-ready:NoExecute 的 taint

	且保证 node 不存在 node.kubernetes.io/unreachable:NoExecute 的 taint;

4、若 NodeReadyCondition 为 unknown，则为 node 添加 node.kubernetes.io/unreachable:NoExecute 的 taint

	且保证 node 不存在 node.kubernetes.io/not-ready:NoExecute 的 taint；
...
/*
上面已经介绍了无论是否启用了 TaintBasedEvictions 特性，需要打 taint 或者驱逐 pod 的 node
都会被放在 zoneNoExecuteTainter 或者 zonePodEvictor 队列中，而 nc.monitorNodeHealth 就是这两个队列中数据的生产者。
nc.monitorNodeHealth 的主要功能是持续监控 node 的状态，
当 node 处于异常状态时更新 node 的 taint 以及 node 上 pod 的状态或者直接驱逐 node 上的 pod，
此外还会为集群下的所有 node 划分 zoneStates 并为每个 zoneStates 设置对应的驱逐速率。
...
2、NodeLifecycleController 根据自身 knownNodeSet 列表中的数据调用 nc.classifyNodes
   将 node 分为三类：added、deleted、newZoneRepresentatives，added 表示新增的，deleted 表示被删除的，
   newZoneRepresentatives 代表该 node 不存在 zoneStates，NodeLifecycleController 会为每一个 node
   划分一个 zoneStates，zoneStates 有 Initial、Normal、FullDisruption、PartialDisruption 四种，
   新增加的 node 默认的 zoneStates 为 Initial，其余的几个 zoneStates 分别对应着不同的驱逐速率；
3、对于 newZoneRepresentatives 中 node 列表，调用 nc.addPodEvictorForNewZone 将 node 添加到对应的的 zoneStates 中，
   然后根据是否启用了 TaintBasedEvictions 特性将 node 分别加入到 zonePodEvictor 或 zoneNoExecuteTainter 列表中，
   若启用了则加入到 zoneNoExecuteTainter 列表中否则加入到 zonePodEvictor 中；
4、对应 added 列表中的 node，首先将其加入到 knownNodeSet 列表中，然后调用 nc.addPodEvictorForNewZone
   将该 node 添加到对应的 zoneStates 中，判断是否启用了 TaintBasedEvictions 特性，若启用了则调用 nc.markNodeAsReachable
   移除该 node 上的 UnreachableTaint 和 NotReadyTaint，并从 zoneNoExecuteTainter 中移除该 node，表示为该 node 进行一次初始化，
   若未启用 TaintBasedEvictions 特性则调用 nc.cancelPodEviction 将该 node 从 zonePodEvictor 中删除；
5、对于 deleted 列表中的 node，将其从 knownNodeSet 列表中删除；
6、遍历所有的 nodes：
7、调用 nc.tryUpdateNodeHealth 获取该 node 的 gracePeriod、observedReadyCondition、currentReadyCondition，
   observedReadyCondition 可以理解为 node 上一次的状态， currentReadyCondition 为本次的状态；
8、检查 node 是否在中断检查中被排除，主要判断当启用 LegacyNodeRoleBehavior 或 NodeDisruptionExclusion 特性时，
   node 是否存在对应的标签，如果该 node 没有被排除，则将其对应的 zone 加入到 zoneToNodeConditions 中；
9、当该 node 的 currentReadyCondition 不为空时，检查 observedReadyCondition，即检查上一次的状态：
9.1、若 observedReadyCondition 为 false，此时若启用了 TaintBasedEvictions 时，为其添加 NotReadyTaint 并且确保 node
   不存在 UnreachableTaint 。若未启用 TaintBasedEvictions 则判断距 node 上一次 readyTransitionTimestamp 的时间是否超过了
   podEvictionTimeout（默认 5 分钟），若超过则将 node 加入到 zonePodEvictor 队列中，最终会驱逐 node 上的所有 pod；
9.2、若 observedReadyCondition 为 unknown，此时若启用了 TaintBasedEvictions 时，则为 node 添加 UnreachableTaint
	 并且确保 node 不会有 NotReadyTaint。若未启用 TaintBasedEvictions 则判断距 node 上一次 probeTimestamp 的时间是否超过了
     podEvictionTimeout（默认 5 分钟），若超过则将 node 加入到 zonePodEvictor 队列中，最终会驱逐 node 上的所有 pod；
9.3、若 observedReadyCondition 为 true 时，此时若启用了 TaintBasedEvictions 时，调用 nc.markNodeAsReachable
     移除 node 上的 NotReadyTaint 和 UnreachableTaint ，若未启用 TaintBasedEvictions 则将 node 从 zonePodEvictor 队列中移除；
     此处主要是判断是否启用了 TaintBasedEvictions 特性，
     然后根据 node 的 ReadyCondition 判断是否直接驱逐 node 上的 pod 还是为 node 打 taint 等待 taintManager 驱逐 node 上的 pod；
...
// 该特性在 v1.16 引入，Alpha 版本，默认为 false，其功能是当 node 存在 node.kubernetes.io/exclude-disruption 标签时，
// 当 node 网络中断时其节点上的 pod 不会被驱逐掉；
...
/*
nc.tryUpdateNodeHealth 会根据当前获取的 node status 更新 nc.nodeHealthMap 中的数据，
nc.nodeHealthMap 保存 node 最近一次的状态，并会根据 nc.nodeHealthMap 判断 node 是否已经处于 unknown 状态。

nc.tryUpdateNodeHealth 的主要逻辑为：

1、获取当前 node 的 ReadyCondition 作为 currentReadyCondition，若 ReadyCondition 为空则此 node 可能未上报 status，
	此时为该 node fake 一个 observedReadyCondition 且其 status 为 Unknown，将其 gracePeriod 设为 nodeStartupGracePeriod，
	否则 observedReadyCondition 设为 currentReadyCondition 且 gracePeriod 为 nodeMonitorGracePeriod，
	然后在 nc.nodeHealthMap 中更新 node 的 Status；
2、若 ReadyCondition 存在，则将 observedReadyCondition 置为当前 ReadyCondition，gracePeriod 设为 40s；
3、计算 node 当前的 nodeHealthData，nodeHealthData 中保存了 node 最近一次的状态，
	包含 probeTimestamp、readyTransitionTimestamp、status、lease 四个字段。
	从 nc.nodeHealthMap 中获取 node 的 condition 和 lease 信息，
	更新 savedNodeHealth 中 status、probeTimestamp、readyTransitionTimestamp，
	若启用了 NodeLease 特性也会更新 NodeHealth 中的 lease 以及 probeTimestamp，
	最后将当前计算出 savedNodeHealth 保存到 nc.nodeHealthMap 中；
4、通过获取到的 savedNodeHealth 检查 node 状态，若 NodeReady condition 或者 lease 对象更新时间超过 gracePeriod，
	则更新 node 的 Ready、MemoryPressure、DiskPressure、PIDPressure 为 Unknown，
	若当前计算出来的 node status 与上一次的 status 不一致则同步到 apiserver，并且更新 nodeHealthMap；
...
2、遍历 zoneToNodeConditions 首先调用 nc.computeZoneStateFunc 计算每个 zone 的状态，

	分为三种 fullyDisrupted（zone 下所有 node 都处于 notReady 状态）、
	partiallyDisrupted（notReady node 占比 >= unhealthyZoneThreshold 的值且 node 数超过三个）、
	normal（以上两种情况之外）。若 newState 不为 stateFullDisruption 将 allAreFullyDisrupted 设为 false，
	将 newState 保存在 newZoneStates 中;
...
5、如果 allAreFullyDisrupted 为 true，allWasFullyDisrupted 为 false，说明从非 FullyDisrupted 切换到了 FullyDisrupted 模式，

	此时需要停止所有 node 的驱逐工作，首先去掉 node 上的 taint 并设置所有zone的对应 zoneNoExecuteTainter 或者
	zonePodEvictor 的 Rate Limeter 为0，最后更新所有 zone 的状态为 FullDisruption；

6、如果 allWasFullyDisrupted 为 true，allAreFullyDisrupted 为 false，说明集群从 FullyDisrupted 变为 非 FullyDisrupted 模式，

	此时首先更新 nc.nodeHealthMap 中所有 node 的 probeTimestamp 和 readyTransitionTimestamp 为当前时间，
	然后调用 nc.setLimiterInZone 重置每个 zone 的驱逐速率；

7、如果 allWasFullyDisrupted为false 且 allAreFullyDisrupted 为false，即集群状态保持为非 FullDisruption 时，

	此时根据 zone 的 state 为每个 zone 设置默认的驱逐速率；
*/
...
statePartialDisruption：如果当前 zone size 大于 nc.largeClusterThreshold（默认为 50，通过--large-cluster-size-threshold设置），

	则设置为 secondaryEvictionLimiterQPS（默认为 0.01，可以通过 --secondary-node-eviction-rate 指定），否则设置为 0；
...
partiallyDisrupted：notReady node 占比 >= unhealthyZoneThreshold 的值
					(默认为0.55，通过--unhealthy-zone-threshold设置)且 notReady node 数超过2个；
normal：以上两种情况之外的；
*/
